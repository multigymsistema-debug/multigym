...
const exerciseImages = {supino:'https://share.zapia.com/fervjq2o4w06oo2ucp34vb',remada:'https://share.zapia.com/9j1lohg7fhgjm59ui1nyz9',agachamento:'https://share.zapia.com/9nsukhm4gt6vh4malreraz',desenvolvimento:'https://share.zapia.com/l71muttabzb3r64urjrr76',triceps:'https://share.zapia.com/zu4c2hqwd4h4mlagxqbm3y'};
const visual = (kind='') => exerciseImages[kind] ? `<div class="exercise-art"><img src="${exerciseImages[kind]}" alt="Ilustração das posições e do movimento" loading="lazy"></div><small class="visual-caption">Observe as duas posições: início e final do movimento</small>` : `<div class="movement-visual"><div class="movement-step"><span>1</span><b>Posição inicial</b></div><div class="movement-arrow">→</div><div class="movement-step active"><span>2</span><b>Movimento controlado</b></div></div><small class="visual-caption">Faça a ida e a volta sem impulso</small>`;
const exerciseGuide = (name='') => { const n=name.toLowerCase(); if(n.includes('supino')) return [visual('supino'),'Deite-se no banco, mantenha os pés firmes no chão, desça a barra até a linha do peito e empurre sem tirar os ombros do banco.']; if(n.includes('remada')) return [visual('remada'),'Mantenha a coluna neutra, puxe a carga em direção ao abdômen e retorne devagar, sem arredondar as costas.']; if(n.includes('agach')) return [visual('agachamento'),'Empurre o quadril para trás, mantenha os joelhos alinhados aos pés e suba pressionando o chão.']; if(n.includes('desenvolvimento')) return [visual('desenvolvimento'),'Comece com os cotovelos abaixo dos punhos, empurre acima da cabeça sem arquear a lombar e desça controlando.']; if(n.includes('rosca')) return [visual('rosca'),'Mantenha os cotovelos colados ao corpo, suba a carga sem balanço e desça até quase estender os braços.']; if(n.includes('tríceps')||n.includes('triceps')) return [visual('triceps'),'Fixe os cotovelos junto ao corpo, estenda os braços completamente sem projetar os ombros e retorne.']; if(n.includes('abdominal')) return [visual('abdominal'),'Contraia o abdômen, mantenha o pescoço neutro e suba apenas até tirar as escápulas do apoio.']; return [visual(),'Use amplitude confortável, postura estável e velocidade controlada. Se tiver dúvida, peça orientação ao professor.']; };

const exerciseInfo = (name='') => { const n=name.toLowerCase(); if(n.includes('supino')) return {muscles:'Peito · tríceps',equipment:'Barra e banco',tempo:'2–1–2',steps:['Deite-se com os pés firmes e escápulas apoiadas.','Desça a barra até a linha do peito, sem perder o controle.','Empurre para cima sem travar os cotovelos.']}; if(n.includes('remada')) return {muscles:'Costas · bíceps',equipment:'Cabo ou máquina',tempo:'2–1–3',steps:['Mantenha a coluna neutra e o peito aberto.','Puxe o equipamento em direção ao abdômen.','Retorne estendendo os braços sem arredondar as costas.']}; if(n.includes('agach')) return {muscles:'Quadríceps · glúteos',equipment:'Barra, Smith ou halter',tempo:'3–1–2',steps:['Apoie o peso e alinhe os pés aos ombros.','Desça levando o quadril para trás e mantendo os joelhos alinhados.','Suba pressionando o chão, sem perder a postura.']}; if(n.includes('desenvolvimento')) return {muscles:'Ombros · tríceps',equipment:'Halteres ou máquina',tempo:'2–1–2',steps:['Comece com os pesos na altura dos ombros.','Empurre verticalmente sem arquear a lombar.','Desça até a posição inicial de forma controlada.']}; if(n.includes('rosca')) return {muscles:'Bíceps',equipment:'Barra, halter ou polia',tempo:'2–1–3',steps:['Mantenha os cotovelos junto ao corpo.','Flexione os braços sem balançar o tronco.','Desça a carga devagar até quase estender os braços.']}; if(n.includes('tríceps')||n.includes('triceps')) return {muscles:'Tríceps',equipment:'Polia e corda/barra',tempo:'2–1–3',steps:['Fixe os cotovelos ao lado do corpo.','Estenda os braços sem projetar os ombros.','Retorne controlando a subida da carga.']}; if(n.includes('abdominal')) return {muscles:'Abdômen',equipment:'Colchonete ou máquina',tempo:'2–1–2',steps:['Mantenha o pescoço neutro e o abdômen contraído.','Eleve o tronco sem puxar a cabeça.','Retorne devagar sem relaxar completamente.']}; return {muscles:'Conforme prescrição',equipment:'Conforme cadastro',tempo:'Controlada',steps:['Confira a posição inicial da ilustração.','Execute sem impulso e sem dor.','Retorne lentamente à posição inicial.']}; };
function applyExerciseGuide(name='') { const guide=exerciseGuide(name); const info=exerciseInfo(name); const art=document.querySelector('#exerciseIllustration'); if(art)art.innerHTML=guide[0]; const text=document.querySelector('#exerciseInstructions span'); if(text)text.textContent=guide[1]; const mus=document.querySelector('#exerciseMuscles'); const equip=document.querySelector('#exerciseEquipment'); const tempo=document.querySelector('#exerciseTempo'); const steps=document.querySelector('#exerciseSteps'); if(mus)mus.textContent=info.muscles;if(equip)equip.textContent=info.equipment;if(tempo)tempo.textContent=info.tempo;if(steps)steps.innerHTML=info.steps.map(x=>`<li>${x}</li>`).join(''); }
...
function renderRealData(data) {
  ...
  const workoutName=document.querySelector('#workoutName'); const workoutMeta=document.querySelector('#workoutMeta'); const homeName=document.querySelector('#homeWorkoutName'); const homeMeta=document.querySelector('#homeWorkoutMeta'); if(workoutName)workoutName.textContent=workout?.name||'Nenhum treino disponível'; if(workoutMeta)workoutMeta.textContent=workout?`${workout.exercises?.length||0} exercícios · treino personalizado`:'A academia ainda não vinculou um treino.'; if(homeName)homeName.textContent=workout?.name||'Nenhum treino disponível'; if(homeMeta)homeMeta.textContent=workout?`${workout.exercises?.length||0} exercícios · treino personalizado`:'A academia ainda não vinculou um treino.';
  ...
  const homePlan=document.querySelector('#homePlan'); const homeExpiry=document.querySelector('#homeExpiry'); const homeStatus=document.querySelector('#homePaymentStatus'); if(homePlan)homePlan.textContent=data.enrollment?.plan_name||'Sem matrícula'; if(homeExpiry)homeExpiry.textContent=data.enrollment?.ends_on?`Vencimento: ${new Date(`${data.enrollment.ends_on}T12:00:00`).toLocaleDateString('pt-BR')}`:'Nenhum vencimento disponível'; if(homeStatus){homeStatus.textContent=data.enrollment?.status||'—';homeStatus.className=data.enrollment?.status==='active'?'paid':'status';}
  const homeAppointments=document.querySelector('#homeAppointments'); if(homeAppointments)homeAppointments.innerHTML=data.appointments?.length?data.appointments.slice(0,2).map(a=>`<article class="info-card"><div><span>Próximo compromisso</span><strong>${a.title}</strong><small>${new Date(a.starts_at).toLocaleString('pt-BR')}</small></div><span class="status">Agendado</span></article>`).join(''):'<p class="muted">Nenhum compromisso agendado.</p>';
  ...
  const appointments = document.querySelector('#agendaItems');
  if (appointments && data.appointments) appointments.innerHTML=data.appointments.length?data.appointments.map(a=>`<div class="appointment"><span>${new Date(a.starts_at).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})}</span><div><strong>${a.title}</strong><small>${new Date(a.starts_at).toLocaleDateString('pt-BR')}</small></div><em>Agendado</em></div>`).join(''):'<p class="muted">Nenhum compromisso agendado.</p>';
}
...
document.addEventListener('click', event => {
  ...
  if (exercise) { currentWorkout = studentHome?.workouts?.find(w => w.id === exercise.dataset.workout); currentExercise = currentWorkout?.exercises?.find(e => String(e.id) === String(exercise.dataset.exercise)) || {id: exercise.dataset.exercise, name: exercise.dataset.name}; document.querySelector('#exerciseTitle').textContent = currentExercise.name || 'Exercício'; document.querySelector('#exerciseProgress').textContent = `${currentWorkout?.name || 'Treino'} · carga prescrita: ${currentExercise.load || 'livre'}`; applyExerciseGuide(currentExercise.name); document.querySelector('#exerciseHistory').innerHTML = '<p class="muted">Carregando histórico...</p>'; fetch(`${API}/student-api/workouts/${currentWorkout.id}/logs`,{headers:{Authorization:`Bearer ${token()}`}}).then(r=>r.json()).then(rows=>{const mine=rows.filter(r=>String(r.exercise_id)===String(currentExercise.id));document.querySelector('#exerciseHistory').innerHTML=mine.length?`<div class="info-card"><div><strong>Últimas séries</strong><small>${mine.slice(0,5).map(r=>`${r.set_number}ª · ${r.weight} kg × ${r.reps}`).join('  |  ')}</small></div></div>`:'<p class="muted">Nenhuma série registrada ainda.</p>';}).catch(()=>{}); showScreen('treino-exercicio'); return; }
