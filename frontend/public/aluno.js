...
document.querySelector('#showDemo')?.addEventListener('click',()=>{const guide=exerciseGuide(currentExercise?.name||'');document.querySelector('#demoFigure').innerHTML=guide[0];document.querySelector('#demoText').textContent=guide[1];document.querySelector('#demoBox').hidden=false;});
...
document.addEventListener('click', event => {
  ...
  if (exercise) { currentWorkout = studentHome?.workouts?.find(w => w.id === exercise.dataset.workout); currentExercise = currentWorkout?.exercises?.find(e => String(e.id) === String(exercise.dataset.exercise)) || {id: exercise.dataset.exercise, name: exercise.dataset.name}; document.querySelector('#exerciseTitle').textContent = currentExercise.name || 'Exercício'; document.querySelector('#exerciseProgress').textContent = `${currentWorkout?.name || 'Treino'} · carga prescrita: ${currentExercise.load || 'livre'}`; const guide=exerciseGuide(currentExercise.name); document.querySelector('#exerciseIllustration').innerHTML=guide[0]; document.querySelector('#exerciseInstructions span').textContent=guide[1]; document.querySelector('#exerciseHistory').innerHTML = '<p class="muted">Carregando histórico...</p>'; fetch(`${API}/student-api/workouts/${currentWorkout.id}/logs`,{headers:{Authorization:`Bearer ${token()}`}}).then(r=>r.json()).then(rows=>{const mine=rows.filter(r=>String(r.exercise_id)===String(currentExercise.id));document.querySelector('#exerciseHistory').innerHTML=mine.length?`<div class="info-card"><div><strong>Últimas séries</strong><small>${mine.slice(0,5).map(r=>`${r.set_number}ª · ${r.weight} kg × ${r.reps}`).join('  |  ')}</small></div></div>`:'<p class="muted">Nenhuma série registrada ainda.</p>';}).catch(()=>{}); showScreen('treino-exercicio'); return; }
  const button = event.target.closest('[data-screen]');
  if (button) { if(button.dataset.screen==='treino-exercicio' && !currentExercise && studentHome?.workouts?.[0]) { currentWorkout=studentHome.workouts[0]; currentExercise=currentWorkout.exercises?.[0]; document.querySelector('#exerciseTitle').textContent=currentExercise?.name||'Exercício'; const guide=exerciseGuide(currentExercise?.name); document.querySelector('#exerciseIllustration').innerHTML=guide[0]; document.querySelector('#exerciseInstructions span').textContent=guide[1]; } showScreen(button.dataset.screen); }
});
