...
test('próximo treino é gerado após conclusão e sem exercício inventado',()=>{assert.match(server,/WORKOUT_COMPLETED/);assert.match(server,/WORKOUT_PRESCRIBED/);assert.match(server,/Treino diário gerado pelo Personal Gym/);assert.match(server,/Carga não definida/)});
