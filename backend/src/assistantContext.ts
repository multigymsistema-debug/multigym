export type AssistantIntent='GREETING'|'QUESTION'|'PROGRESS'|'RECOMMENDATION'|'MEAL_ANALYSIS'|'MEAL_REGISTRATION'|'HYDRATION'|'WORKOUT_INFO'|'EXERCISE_TECHNIQUE'|'REST'|'TRAINING_PROGRESS'|'WORKOUT_COMPLETION'|'NEXT_WORKOUT'|'MOTIVATION'|'GOAL'|'TROUBLESHOOTING'|'SAFETY'|'PROFESSIONAL_REFERRAL';
...
export function detectAssistantIntent(message:string,assistant:'nutrition'|'personal',image=false):AssistantIntent{
  ...
  if(assistant==='nutrition'){
    ...
    if(/como estou|evolu|progresso|resumo|hoje/.test(m))return 'PROGRESS';
  }else{
...
export function buildNutriContext(raw:any,intent:AssistantIntent,message:string,conversation:any[]=[],event?:any){
  const today=raw.today||new Date().toISOString().slice(0,10),meals=Array.isArray(raw.meals)?raw.meals:[],todayMeals=meals.filter((m:any)=>String(m.meal_date).slice(0,10)===today),sum=(key:string)=>todayMeals.reduce((n:any,m:any)=>n+Number(m[key]||0),0),nutritionNeeds=['MEAL_ANALYSIS','MEAL_REGISTRATION','HYDRATION','RECOMMENDATION','PROGRESS','GOAL','QUESTION','PROFESSIONAL_REFERRAL'].includes(intent);
  const context:any={assistant:'NutriGym',intent,question:message.slice(0,500),date_time:now(),event:event||null,student:compactStudent(raw.student),profile:compactProfile(raw.profile,'nutrition'),goals:nutritionNeeds?(raw.goals||[]):[],conversation:safeTurns(conversation)};
  if(nutritionNeeds)context.nutrition={today,meals:todayMeals.slice(0,10),recent_meals:meals.slice(0,8),totals_today:{calories:sum('calories'),protein_g:sum('protein_g'),carbs_g:sum('carbs_g'),fat_g:sum('fat_g')},hydration:(raw.hydration||[]).slice(0,8),checkin:raw.checkin||null,checkin_history:(raw.checkinHistory||[]).slice(0,6),plan:raw.plan||null,memories:(raw.memories||[]).slice(0,10)};
...
export function buildPersonalContext(raw:any,intent:AssistantIntent,message:string,conversation:any[]=[],event?:any){
  ...
  const context:any={assistant:'Personal Gym',intent,question:message.slice(0,700),date_time:now(),event:event||null,student:compactStudent(raw.student),profile:compactProfile(raw.profile,'personal'),conversation:safeTurns(conversation)};
  if(needsTraining)context.training={active_workouts:(raw.workouts||[]).slice(0,3),history:(raw.workoutHistory||[]).slice(0,12),logs:(raw.logs||[]).slice(0,30),assessments:(raw.assessments||[]).slice(0,8)};
